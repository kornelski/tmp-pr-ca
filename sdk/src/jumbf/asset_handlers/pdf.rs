...
mod tests {
    ...
    // macro provides compile time concatenation of test path
    macro_rules! include_test_bytes {
        ($p:expr) => {{
            include_bytes!(std::concat!("../../../tests/fixtures/", $p))
        }};
    }
    ...
    fn test_loads_pdf_from_bytes() {
        let bytes = include_test_bytes!("basic.pdf");
    ...
    fn test_loads_pdf_from_bytes_with_invalid_file() {
        let bytes = include_test_bytes!("XCA.jpg");
        let pdf_result = Pdf::from_bytes(bytes);
    ...
    fn test_is_password_protected() {
        let bytes = include_test_bytes!("basic-password.pdf");
    ...
    fn test_adds_embedded_file_spec_to_pdf_stream() {
        let bytes = include_test_bytes!("express.pdf");
        let mut pdf = Pdf::from_bytes(bytes).unwrap();
    ...
    fn test_write_manifest_as_annotation() {
        let mut pdf = Pdf::from_bytes(include_test_bytes!("express.pdf")).unwrap();
        assert!(!pdf.has_c2pa_manifest());
    ...
    fn test_write_manifest_bytes_to_pdf_with_existing_annotations() {
        let mut pdf = Pdf::from_bytes(include_test_bytes!("basic-annotation.pdf")).unwrap();
        pdf.write_manifest_as_annotation(vec![10u8, 20u8]).unwrap();
        assert!(pdf.has_c2pa_manifest());
    }
    ...
    fn test_add_manifest_to_embedded_files() {
        let mut pdf = Pdf::from_bytes(include_test_bytes!("basic.pdf")).unwrap();
    ...
    fn test_add_manifest_to_embedded_files_attachments_present() {
        let mut pdf = Pdf::from_bytes(include_test_bytes!("basic-attachments.pdf")).unwrap();
        pdf.write_manifest_as_embedded_file(vec![10u8, 20u8])
            .unwrap();
    ...
    fn test_read_manifest_bytes_from_pdf_without_bytes_returns_none() {
        let pdf = Pdf::from_bytes(include_test_bytes!("basic.pdf")).unwrap();
        assert!(!pdf.has_c2pa_manifest());
    ...
    fn test_read_xmp_on_pdf_with_none() {
        let pdf = Pdf::from_bytes(include_test_bytes!("basic-no-xmp.pdf")).unwrap();
